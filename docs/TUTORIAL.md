...
no server of your own, no data to load. You put widgets on a grid, point each one at a subject, and
it fetches from the Wikimedia APIs directly.

Open <https://wikibento.toolforge.org/> and you get a **starter board of three widgets**: pageviews for
...
Every widget has the same top bar:

- **ⓘ** — where this widget's data comes from (useful when a number surprises you)
- **⚙** — configure it: the subject, the wiki, the display mode
- **↻** — refresh now
- **✕** — remove the widget
- the small **name chip** next to the title is the widget's identity. Other widgets can refer to a widget
...
- **Starter set** — the same three-widget board you began with (Main Page pageviews, a link count, the
  largest Wikipedias).
- **Blank board** — nothing at all, just the empty grid, ready for the widgets you choose.

**Cancel** changes nothing. A blank board is *remembered*: reloading keeps it empty rather than
bringing the starter widgets back.

> ⚠️ **Export first if you want to keep the current board.** Reset clears both the widgets and the
...
0. To start from an empty grid, click the **✕** on each widget's top bar — that removes it. (It is also
   how you make room: the widget added below is easier to read on a board of its own.)

1. Click **+ Add Widget**. A picker opens listing every widget type, grouped by category
...
2. Type `pageviews` and pick **Article Pageviews**. The widget appears immediately — it starts on a
   placeholder subject.
...
4. Type or paste the **exact article title** (spaces are fine — `Albert Einstein`, not
   `Albert_Einstein`), pick the wiki, and the widget fetches real data: the 30-day total plus a daily
   bar chart.

Changes apply as you make them. If a widget shows an error, the usual cause is a subject that does not
exist on that wiki — check the ⓘ panel for the exact API call the widget made.

## 5. Move and resize

- **Drag by the top bar** to move a widget. The rest of the board reflows around it.
...
Click **⬇ Export**. Your browser downloads `dashboard.json` — a small file containing every widget,
...
text support: every caption, badge, title card and end card is rendered by a real browser into a PNG,
composited
