...
- **Desktop vs mobile view:** `mobile: true` appends `?useskin=minerva` on the **same domain** — the canonical stateless way to request the mobile (Minerva) skin. ⚠️ **The m. subdomains are retired**: `en.m.wikipedia.org` and `commons.m.wikimedia.org` both 301 to their desktop domains (verified 2026-08-13), so host-swapping no longer works. The desktop skin is responsive regardless and collapses its sidebar below ~720 px.
