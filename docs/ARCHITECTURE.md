...
│           └── WidgetContent  (dispatch on def.renderer — ~24 named cards,
...
**Commons Impact Metrics API (`wikimedia.org/api/rest_v1/metrics/commons-analytics/`) — dependency of all 9 CIM widgets.** Migration-risk watch (verified 2026-09-01): the whole `/metrics` AQS family is slated to move off RESTBase to a bespoke API-Gateway service (Phabricator **T263489**, AQS 2.0) — the RESTBase `?spec` already lists ZERO commons-analytics routes while the endpoints still work, and wrong-shaped requests return `{detail: "invalid route"}` (route-404, NOT the "not loaded yet" data-404). Route contracts come from the service's own reference doc (`gitlab.wikimedia.org/repos/generated-data-platform/aqs/analytics-api/-/raw/main/reference/commons.md`, SpecView entries); the doc.wikimedia.org page renders headings only. Verify after any WMF analytics announcement: one GET per route family (`category-metrics-snapshot/{cat}/{start}/{end}` — no `/monthly/` segment; `top-viewed-categories-monthly/{scope}/{wiki}/{YYYY}/{MM}`), the `{context, items}` envelope, and that the global leaderboard still exists (it's also `latestCimMonth()`'s publish probe — if it dies, month resolution must be reworked). Related: this API family publishes MONTHLY and lags days into each month — widgets must resolve months via `latestCimMonth()`, never the calendar (see tests/cim-latest-month.test.mjs).
