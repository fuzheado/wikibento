...
| **IA Book** | 📖 | `iiif.archive.org` — Presentation v3 manifest, Image API v3, Content Search (CORS ✓, no key) | A **scanned Internet Archive book**, page by page: turn, zoom to read, jump from a 15-page thumbnail strip, **search inside it** (each hit names its page and shows the matched words boxed on the page), read that page's OCR text, and open the PDF/EPUB/OCR. Page count comes from the manifest's canvases, not the item metadata |
