...
| **Gallery** | 🖼️ | One widget, **four sources** → one row shape: REST `/page/media-list` + `imageinfo` (an article) · `prop=revisions` wikitext → `<gallery>` blocks (a Commons gallery page) · `list=categorymembers` + `imageinfo` (a wiki category) · batched `imageinfo` (a pasted list) | Images from an article, a **gallery page** (its own captions and order — Commons' curated layer), a wiki category (alphabetical, newest, random or largest, with the category total when a card shows part of it), or a list you paste — grid or list, optional section grouping, clickable into a new tab or out to another widget |
