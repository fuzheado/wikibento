...
- [docs/TUTORIAL-VIDEO-STATUS.md](docs/TUTORIAL-VIDEO-STATUS.md) — the tutorial-video pipeline: what each script does, what is verified working, the five blockers (local ffmpeg has no text filters; Debian font paths; Linux-only output defaults; no TTS step; `SCRIPT.md`/`scenes.json` desync), and the order to fix them
