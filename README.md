...
- **✨ Ask** — describe what you want in plain language ("random sampling of images from a category") and get
  ...
  from the registry.
- **📰 Live Wikipedia boxes** — render a *template* with the wiki's own markup and its own styles: the five Main
  Page boxes (**In the news**, Did you know, Today's featured article, the selected anniversaries, Picture of the
  day) or any other, including dated ones that keep themselves current. A box's links open in a new tab; set
  *Links in the box* to **send to the board** and a click publishes **the page the reader chose** for another
  widget to load or draw. [docs/WIRING-BOARDS.md](docs/WIRING-BOARDS.md) is the wiring guide.
- **🌍 Every wiki, chosen quickly** — one picker for all 364 projects, ordered by the ones you use, your default
  wiki, a curated shortlist, then everything else; searchable by name, code, script or English name. ⚙ **Settings**
  holds your wiki and the recent list.
...
└── src/
    ...
    ├── components/            # AddWidgetPanel, ImportPanel, SharePanel, AboutPanel, BoardNotice, ProjectField,
    │                          # SettingsPanel, ErrorBoundary
    ├── lib/
